...
export function getWorklet(ac, processor, params, config) {
  try {
    ...
    // If AudioWorklet is not available (e.g. worklets failed to load),
    // return a pass-through gain node so the audio chain still works
    console.warn(`[superdough] AudioWorklet '${processor}' unavailable, bypassing effect:`, err.message);
    return gainNode(1);
  }
}
